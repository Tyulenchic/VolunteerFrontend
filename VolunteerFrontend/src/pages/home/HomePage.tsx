...
// ─── Helpers ─────────────────────────────────────────────────────────────────
type ModalT = 'login' | 'register' | 'feedback' | 'event' | null;
      ...
      <>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 relative">
              <div className="hidden lg:block absolute top-8 left-[calc(12.5%+1rem)] right-[calc(12.5%+1rem)] h-px bg-gradient-to-r from-blue-200 via-blue-400 to-blue-200 z-0" />
               {[
                 { n: 1, icon: 'fa-user-plus',      t: 'Зарегистрируйтесь',    d: 'Заполните простую форму за 5 минут' },
                 { n: 2, icon: 'fa-id-card',         t: 'Заполните профиль',    d: 'Расскажите о себе и своих интересах' },
                 { n: 3, icon: 'fa-search',           t: 'Выберите мероприятие', d: 'Просмотрите календарь событий' },
                 { n: 4, icon: 'fa-heart',            t: 'Примите участие',      d: 'Приходите и делайте мир лучше!' },
               ].map(s => (
                   <div key={s.n} className="relative bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md hover:border-blue-100 transition-all z-10 text-center">
        ...
        <section id="feedback" className="py-16 sm:py-20 bg-gray-50">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-xl text-center">
            ...
            <button onClick={() => setModal('feedback')} className="inline-flex items-center px-8 py-3.5 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition shadow-md text-sm">
        ...
        <Modal isOpen={modal === 'feedback'} onClose={close} title="Обратная связь">
